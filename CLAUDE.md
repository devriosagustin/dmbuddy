...
**Security Rules (2026-09-03):** `database.rules.json` + `firebase.json` en la raíz proponen una regla base: nada es legible/escribible en la raíz de la base, solo `sessions/$code` (conociendo el código exacto — nadie puede listar `/sessions` para descubrir códigos ajenos). Es la regla mínima razonable para el modelo "quien tiene el código, entra" que ya usa la app; no valida el *shape* de cada nodo todavía. **No están aplicadas en el proyecto de Firebase real** — hay que pushearlas manualmente (`firebase deploy --only database` con el CLI logueado, o pegar el contenido de `database.rules.json` en la consola de Firebase, pestaña Rules del Realtime Database) ya que esta sesión no tiene credenciales del proyecto `dmbuddy-10a10`. Importante: como es un único proyecto Firebase compartido por *todos* los que usan la app publicada (no es local por usuario), si las rules actuales siguen en modo test (abiertas), cualquiera que sepa la URL de la app podía leer/escribir sesiones ajenas con solo adivinar o interceptar un código — vale la pena aplicar esto pronto.
