...
/** Casillas dentro de un radio (círculo) en pies. */
export const cellsInSphere = (cx: number, cy: number, radiusFeet: number): MapCell[] => {
...
export const cellsInLine = (cx: number, cy: number, tx: number, ty: number, lengthFeet: number): MapCell[] => {
...
export const cellsInCone = (cx: number, cy: number, tx: number, ty: number, lengthFeet: number): MapCell[] => {
  const L = feetToCells(lengthFeet);
  const vecX = tx - cx;
  const vecY = ty - cy;
  const len = Math.hypot(vecX, vecY);
